...
import { MAIN_URL, REGISTER_URL, SINGIN_URL, CONTACT_URL, FINANCE_URL, BUY_URL, COMUNITY_URL, GAMES_URL } from '../../utils/links';
...
export default function Header({ auth, show, setShow, setToken }: HeaderProps) {
  ...
  return (
    <header className={styles.container}>
      <div
        className={`${styles.top} ${isMobileSearch ? styles.top_search : ""}`}
        ref={refHeader}>
        ...
        {pathname === "/" || pathname.includes("game") ? (
          isMobileSearch ? (
            <Search
              ...
              onBlur={() => setIsMobileSearch(false)}
              value={searchValue}
        ...
        {!show ? (
          <svg
            ...
            <path
              d="M8 23H28M8 13H28H8ZM8 18H28H8Z"
              stroke="white"
              strokeMiterlimit="10"
              strokeLinecap="round"
        ...
        <div
          ...
          {pathname === "/" || pathname.includes("game") ? (
            isPcSearch ? (
              ...
              <img
                src={searchIcon}
                className={styles.openSearchIcon_menu}
                onClick={() => setIsPCSearch(true)}
                alt="search"
              />
            )
          ) : undefined}
